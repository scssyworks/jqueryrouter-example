  ...
  bindEvents() {
    route((routeData) => {
      if (!routeData.hash) {
        ...
        const target = _cache.homeLinks
          .filter(`[data-route="${rootRoute}"]`)
          .addClass('active')
          .data('target');
      ...
      if (routeData.hash) {
        if (/accordion/.test(routeData.route)) {
          ...
          _cache.accordionBtns
            .filter(`[data-route="${routeData.route}"]`)
            .attr('aria-expanded', true);
          _cache.accordionSections.removeClass('show');
          _cache.accordionSections
            .filter(`[data-route="${routeData.route}"]`)
            .addClass('show');
        }
      }
    ...
    route('/demos/demo2/modalroute/:progress', function (config) {
      ...
      _cache.modalNextBtn.text(
        progress === _cache.wizardSections.length
          ? 'Close this window'
          : 'Go to next step'
      );
    ...
    route('/names/Peter', function () {
      console.log('Page refresh: Peter');
    });
    route('/names', function () {
      console.log('Page refresh: All names');
    });
